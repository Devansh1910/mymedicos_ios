...
class GrandTestNeetssViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, ExamQuizLiveTableViewCellDelegate, BottomSheetDelegate, SWGTUpcomingBottomSheetDelegate {
    ...
    func fetchNeetssValue(completion: @escaping (String?) -> Void) {
        ...
        ref.child("profiles").child(phoneNumber).child("Neetss").observeSingleEvent(of: .value, with: { snapshot in
            ...
            print("Error fetching Neetss value: \(error.localizedDescription)")
            completion(nil)
        }
    }
    ...
    func fetchLiveQuizzes() {
        ...
        fetchNeetssValue { [weak self] neetssValue in
            guard let self = self, let speciality = neetssValue else {
                self?.activityIndicator.stopAnimating()
                self?.showAlert(title: "Error", message: "Failed to fetch speciality information.")
                return
            }

            let db = Firestore.firestore()
            let quizCollection = db.collection("Neetss").document(speciality).collection("Weekley")
            ...
            query.getDocuments { (querySnapshot, err) in
                ...
                for document in documents {
                    ...
                    self.quizData.append((documentID: documentID, data: title, isBookmarked: false, thumbnail: thumbnail))
    ...
    func fetchUpcomingQuizzes() {
        activityIndicator.startAnimating()
        fetchNeetssValue { [weak self] speciality in
            guard let self = self, let speciality = speciality else {
                self?.activityIndicator.stopAnimating()
                self?.showAlert(title: "Error", message: "Failed to fetch speciality information.")
                return
            }
            ...
            query.getDocuments { (querySnapshot, err) in
                ...
                if let err = err {
                    ...
                    for document in documents {
                        ...
                        let questionsData = data["Data"] as? [Any] ?? []
                        let numberOfQuestions = questionsData.count
                        let endDate = (data["to"] as? Timestamp)?.dateValue()
                        let isBookmarked = false
                        let documentInfo = "\(title); \(numberOfQuestions) MCQ's; \(type)"
                        self.quizData.append((documentID: documentID, data: documentInfo, isBookmarked: isBookmarked, thumbnail: thumbnail))
                        self.quizStartDates.append(endDate)
                    }
                } else {
                    self.handleNoData()
                }
