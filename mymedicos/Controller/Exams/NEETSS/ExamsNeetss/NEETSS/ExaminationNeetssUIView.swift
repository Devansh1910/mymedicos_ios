...
protocol ExaminationNeetssUIViewDelegate: AnyObject {
    func navigateToExamPortal(withTitle title: String, examID: String)
}

class ExaminationNeetssUIView: UIView {
    weak var delegate: ExaminationNeetssUIViewDelegate?
    ...
    // NEET Button
    private lazy var neetButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("Default Exam", for: .normal)  // Default title
    ...
    private func fetchData() {
        ...
        dbRef.child("profiles").child(userPhoneNumber).child("Neetss").observeSingleEvent(of: .value, with: { [weak self] snapshot in
            guard let self = self else { return }

            if let title = snapshot.value as? String {
                self.updateButtonTitle(title)
                self.fetchExams(title)
            } else {
    ...
    private func updateButtonTitle(_ title: String) {
        DispatchQueue.main.async { [weak self] in
            self?.neetButton.setTitle(title, for: .normal)
        }
    }
    
    private func fetchExams(_ category: String) {
        let db = Firestore.firestore()
        let today = Date()

        // Query documents from Firestore
        db.collection("Neetss").document(category).collection("Weekley")
            ...
            .getDocuments { [weak self] (querySnapshot, error) in
                ...
                for document in documents {
                    let fromTimestamp = document.data()["from"] as? Timestamp
                    let toTimestamp = document.data()["to"] as? Timestamp
                    let fromDate = fromTimestamp?.dateValue()
                    let toDate = toTimestamp?.dateValue()

                    // Ensure both dates are present and check if today's date is within range
                    if let fromDate = fromDate, let toDate = toDate, today >= fromDate && today <= toDate {
                        let title = document.data()["title"] as? String
                        let type = document.data()["type"] as? String
                        let examID = document.data()["qid"] as? String

                        // Check required fields are not nil
                        if let title = title, let type = type, let examID = examID {
                            DispatchQueue.main.async {
                                self.questionLabel.text = title
                                let iconName = (type == "A" ? "lock.fill" : "lock.open.fill")
