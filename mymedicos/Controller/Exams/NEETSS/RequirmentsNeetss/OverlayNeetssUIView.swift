...
class OverlayNeetssUIView: UIView {
    
    // Views declarations
    private var dragHandle: UIView!
    private var carouselView: CarousleNeetssUIView!
    private var examinationTitleLabel: UILabel!
    private var examinationView: ExaminationNeetssUIView!
    ...
    private func setupCarouselView(contentView: UIView) {
        carouselView = CarousleNeetssUIView(frame: CGRect.zero)
    ...
    private func setupExaminationView(contentView: UIView) {
        examinationView = ExaminationNeetssUIView(frame: CGRect.zero)
    ...
    private func fetchPlansAndLayout() {
        ...
        if let cachedPlans = cache.object(forKey: cacheKey as NSString) {
            ...
            db?.collection("Plans").document("NEET SS").collection("Subscriptions").getDocuments { [weak self] (querySnapshot, error) in
