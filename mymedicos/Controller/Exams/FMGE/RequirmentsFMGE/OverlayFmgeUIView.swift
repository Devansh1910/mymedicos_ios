...
class OverlayFmgeUIView: UIView {
    
    // Views declarations
    private var dragHandle: UIView!
    private var carouselView: CarousleFMGEUIView!
    private var examinationTitleLabel: UILabel!
    private var examinationView: ExaminationFmgeUIView!
    ...
    private func setupCarouselView(contentView: UIView) {
        carouselView = CarousleFMGEUIView(frame: CGRect.zero)
    ...
    private func setupExaminationView(contentView: UIView) {
        examinationView = ExaminationFmgeUIView(frame: CGRect.zero)
    ...
    private func fetchPlansAndLayout() {
        ...
        if let cachedPlans = cache.object(forKey: cacheKey as NSString) {
            ...
            db?.collection("Plans").document("FMGE").collection("Subscriptions").getDocuments { [weak self] (querySnapshot, error) in
    ...
    private func layoutPlansFromCache(_ cachedPlans: NSArray) {
        ...
        for plan in cachedPlans as! [PlanData] {
            ...
            planView.configure(with: plan) // Pass PlanData which now includes documentId
